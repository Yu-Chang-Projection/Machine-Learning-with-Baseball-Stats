...
# functions
def save_model(regr):
    filename = '../Best_Models/Linear_BA_(10).pkl'
...
for j in range(20):
    ...
    X = train_df[["LD%","GB%","FB%","HR/FB","xBA","xBACON","SweetSpot%","Barrel%","HardHit%"]]
    y = train_df["BABIP"]
    ...
    for i in range(0, test_df.shape[0]):  # iterate thru all players, shape[0]: the row count of df
        predictBA = regr.predict([test_df.iloc[i][9:18]])  # feed in required data
        pid = test_df.iat[i, 0]
        xBA = round(predictBA[0]*100, 3)
        BA = round(test_df.at[pid, 'BABIP']*100, 2)
