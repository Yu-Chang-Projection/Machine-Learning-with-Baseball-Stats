...
def training_function(selected_stats, target_stat, type="L", test_size=0.20, prev_R2 = 0.15, loops = 100):

    def save_model(regr):
        filename = f'../Best_Models/Linear_{target_stat}_({str(int(test_size*100))}).pkl'
    ...
    print(f"MAX R2: {max(result_df['R2'].to_list())}", f"AVG R2: {sum(result_df['R2'].to_list())/loops}", f"{sum(result_df['MAE'].to_list())/loops}")