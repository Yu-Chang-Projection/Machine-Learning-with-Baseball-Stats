...
# main fuction
def training_function(selected_stats, target_stat, test_size=0.15, prev_R2 = 0.5, loops = 20):
    ...
    for j in range(loops):
        ...
        for i in range(0, test_df.shape[0]):  # iterate thru all players, shape[0]: the row count of df
            pid = test_df.iat[i, 0]
            predictBA = regr.predict([columns.loc[pid]])  # feed in required data
            expected_stat = round(predictBA[0], 3)
        ...
        if R2 > prev_R2:
            # save_model(regr)
            prev_R2 = R2
            # regr_df.to_csv(f"Linear_every_{target_stat}.csv")

    # result_df.to_csv(f"Linear_{target_stat}.csv")
    print(f"MAX R2: {max(result_df['R2'].to_list())}", f"AVG R2: {sum(result_df['R2'].to_list())/loops}")

# parameters
selected_stats = ["LD%","GB%","FB%","IFFB%","SweetSpot%","Barrel%","HardHit%","Pull%","Cent%","Oppo%","Spd","LDFB_EV"]
target_stat = "BABIP"

# run function

for i in range(12,1,-1):
    for stat_combo in itertools.combinations(selected_stats,i):
